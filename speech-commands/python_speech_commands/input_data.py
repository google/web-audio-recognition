...
#        dct_coefficient_count=model_settings['dct_coefficient_count'])
  ...
  def get_data(self, how_many, offset, model_settings, background_frequency,
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      # Compute noisy input audio
      y = sess.run(self.mfcc_, feed_dict=input_dict).flatten()

      # Compute melspectrogram features
      import librosa
      spec = librosa.feature.melspectrogram(
        y=y,
        sr=model_settings['sample_rate'],
        n_fft=model_settings['window_size_samples'],
        hop_length=model_settings['window_stride_samples'],
        power=2.0,
        n_mels=model_settings['dct_coefficient_count'],
        fmin=30.0,
        htk=True)

      # NOTE: be wary that the data is flattened here and not in the graph itself!!
      # Run the graph to produce the output audio.
      data[i - offset, :] = spec.flatten()
